...
describe('renderAddPage – per-field inline errors (02-§6.5)', () => {
  it('ILE-01: does not render a #form-errors block', () => {
    assert.ok(
  ...
  for (const field of REQUIRED_FIELDS) {
    it(`ILE-02-${field}: renders a .field-error span for "${field}"`, () => {
      const html = render();
    ...
    it(`ILE-03-${field}: input "${field}" has aria-describedby pointing to its error span`, () => {
      const html = render();
      const errId = `err-${field}`;
  ...
  it('ILE-04: field-error spans are hidden by default', () => {
    const html = render();
