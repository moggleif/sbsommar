...
| `07-§6.135` | The in-progress row (`.is-now`) on the weekly schedule and today view has a terracotta wash and left accent bar, scoped away from the display view | 07-design/components.md §6 (Event / schedule items) | CSS-39 | `source/assets/cs/style.css` – `body:not(.display-mode) .event-row.is-now`; `source/assets/js/client/schema-status.js` (schema.html), `events-today.js` (idag.html) | covered |
