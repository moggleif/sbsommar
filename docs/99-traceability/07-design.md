...
| `07-§6.134` | Ended weekly-schedule rows (`.is-past`) have a light-grey background and dimmed time/title | 07-design/components.md §6 (Event / schedule items) | CSS-38 | `source/assets/cs/style.css` – `body:not(.display-mode) .event-row.is-past`; `source/assets/js/client/schema-status.js` | covered |
| `07-§6.135` | The in-progress weekly-schedule row (`.is-now`) has a terracotta wash and left accent bar, scoped away from the display view | 07-design/components.md §6 (Event / schedule items) | CSS-39 | `source/assets/cs/style.css` – `body:not(.display-mode) .event-row.is-now`; `source/assets/js/client/schema-status.js` | covered |
