...
| `02-§4.22` | Display view re-classifies activities every minute without reloading: ended activities are de-emphasised, the in-progress one is highlighted | 02-requirements/schedule-and-detail.md §4; 07-design/components.md §6.40 | — (manual: open `/live.html` on a day with events spanning the current time; confirm past rows fade and the ongoing row is accented, and that the state updates as the minute rolls over) | `source/assets/js/client/events-today.js` – `classifyRows()` adds `.is-past`/`.is-now`, `tickClassify()` re-runs each minute; `source/assets/cs/style.css` – `body.display-mode .event-row.is-past`/`.is-now` | implemented |
