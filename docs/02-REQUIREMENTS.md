...
- On successful validation, the pipeline must build the site and deploy only the four
  ...
  `events.json`. <!-- 02-§22.11 -->
- No other files may be touched by this pipeline's FTP upload step. <!-- 02-§22.12 -->
- This deployment must happen while the PR is open (before auto-merge), so the updated
  schedule is visible to participants without waiting for the full site deploy. <!-- 02-§22.13 -->
