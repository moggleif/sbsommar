...
| `CL-§9.4` | For data-only commits (YAML files only), CI runs build only — lint and tests are skipped; full deploy is not triggered | 04-OPERATIONS.md (CI/CD Workflows) | — | `.github/workflows/ci.yml` – data-only path check; `.github/workflows/deploy.yml` – `paths-ignore: source/data/**.yaml` | implemented |
...
| `02-§21.12` | Accordion header shows camp name as primary text with date range and location in subdued gray text | 03-ARCHITECTURE.md §4a | — | — | gap |
| `02-§21.13` | Header date range is formatted as `D–D månadsnamn YYYY` | 03-ARCHITECTURE.md §4a | — | — | gap |
| `02-§21.14` | Header location follows date range, separated by `·` | 03-ARCHITECTURE.md §4a | — | — | gap |
| `02-§21.15` | On narrow viewports the header metadata may wrap below the camp name but remains visually subdued | 03-ARCHITECTURE.md §4a | — (manual: resize viewport) | — | gap |
| `02-§21.16` | When a camp accordion is expanded, its timeline dot is visually highlighted (larger, accent color) | 03-ARCHITECTURE.md §4a | — (manual: open an accordion and verify dot change) | — | gap |
| `02-§21.17` | When the accordion is collapsed the dot returns to default size | 03-ARCHITECTURE.md §4a | — (manual: close accordion and verify dot) | — | gap |
| `02-§21.18` | Facebook logo image replaces text button when `link` is non-empty | 03-ARCHITECTURE.md §4a | — | — | gap |
| `02-§21.19` | Facebook logo is placed at top of panel content near camp metadata | 03-ARCHITECTURE.md §4a | — | — | gap |
| `02-§21.20` | Facebook link opens in a new tab with `target="_blank"` and `rel="noopener noreferrer"` | 03-ARCHITECTURE.md §4a | — | — | gap |
| `02-§21.21` | Facebook logo image has accessible `alt` text | 03-ARCHITECTURE.md §4a | — | — | gap |
| `02-§21.22` | Each expanded accordion displays the camp's events from its YAML file, loaded at build time | 03-ARCHITECTURE.md §4a | — | — | gap |
| `02-§21.23` | Events are grouped by date with day headings (e.g. "måndag 3 augusti 2025") | 03-ARCHITECTURE.md §4a | — | — | gap |
| `02-§21.24` | Within each date, events are sorted by start time ascending | 03-ARCHITECTURE.md §4a | — | — | gap |
| `02-§21.25` | Event rows use the same visual format as the weekly schedule: time, title, metadata | 03-ARCHITECTURE.md §4a | — | — | gap |
| `02-§21.26` | Day headings are plain headings, not collapsible | 03-ARCHITECTURE.md §4a | — | — | gap |
| `02-§21.27` | Event rows are flat (not collapsible) — only time, title, and metadata shown | 03-ARCHITECTURE.md §4a | — | — | gap |
| `02-§21.28` | If a camp has no events in its YAML file, the event list section is omitted | 03-ARCHITECTURE.md §4a | — | — | gap |
| `02-§21.29` | Archive page uses the same typography scale, color tokens, and spacing tokens as the rest of the site | 03-ARCHITECTURE.md §4a, 07-DESIGN.md §7 | — (manual: visual comparison) | — | gap |
| `02-§21.30` | Event list styling matches the weekly schedule page in font size, weight, and color | 03-ARCHITECTURE.md §4a | — (manual: visual comparison) | — | gap |
...
19 requirements added for archive page improvements (02-§21.12–21.30):
  all 19 are gaps pending implementation.
```
