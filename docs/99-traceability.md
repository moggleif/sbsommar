...
| `02-§44.1` | PHP API implements POST /api/add-event | 03-ARCHITECTURE.md §21 | PHP unit tests | `api/index.php`, `api/src/GitHub.php` | gap |
| `02-§44.2` | PHP API implements POST /api/edit-event | 03-ARCHITECTURE.md §21 | PHP unit tests | `api/index.php`, `api/src/GitHub.php` | gap |
| `02-§44.3` | Both endpoints return JSON with Content-Type: application/json | 03-ARCHITECTURE.md §21 | PHP unit tests | `api/index.php` | gap |
| `02-§44.4` | GET /api/health returns status JSON | 03-ARCHITECTURE.md §21 | PHP unit tests | `api/index.php` | gap |
| `02-§44.5` | All §10 validation rules replicated in PHP | 03-ARCHITECTURE.md §21 | PHP unit tests | `api/src/Validate.php` | gap |
| `02-§44.6` | Camp date range validation enforced | 03-ARCHITECTURE.md §21 | PHP unit tests | `api/src/Validate.php` | gap |
| `02-§44.7` | Past-date blocking enforced | 03-ARCHITECTURE.md §21 | PHP unit tests | `api/src/Validate.php` | gap |
| `02-§44.8` | Edit requests require non-empty id field | 03-ARCHITECTURE.md §21 | PHP unit tests | `api/src/Validate.php` | gap |
| `02-§44.9` | Time-gating enforced (opens_for_editing..end_date + 1 day) | 03-ARCHITECTURE.md §21 | PHP unit tests | `api/src/TimeGate.php` | gap |
| `02-§44.10` | HTTP 403 with Swedish error when outside editing period | 03-ARCHITECTURE.md §21 | PHP unit tests | `api/index.php` | gap |
| `02-§44.11` | Commits new events via GitHub Contents API (ephemeral branch + PR + auto-merge) | 03-ARCHITECTURE.md §21 | PHP unit tests | `api/src/GitHub.php` | gap |
| `02-§44.12` | Edit requests patch existing event in YAML | 03-ARCHITECTURE.md §21 | PHP unit tests | `api/src/GitHub.php`, `api/src/Yaml.php` | gap |
| `02-§44.13` | Active camp resolved from camps.yaml on GitHub | 03-ARCHITECTURE.md §21 | PHP unit tests | `api/src/ActiveCamp.php` | gap |
| `02-§44.14` | YAML serialisation compatible with data contract | 03-ARCHITECTURE.md §21 | PHP unit tests | `api/src/Yaml.php` | gap |
| `02-§44.15` | Reads and writes sb_session cookie (JSON array, URL-encoded) | 03-ARCHITECTURE.md §21 | PHP unit tests | `api/src/Session.php` | gap |
| `02-§44.16` | Cookie attributes match Node.js (Path, Max-Age, Secure, SameSite, Domain) | 03-ARCHITECTURE.md §21 | PHP unit tests | `api/src/Session.php` | gap |
| `02-§44.17` | Edit requests verify event ID in session cookie; 403 if not | 03-ARCHITECTURE.md §21 | PHP unit tests | `api/index.php` | gap |
| `02-§44.18` | Cookie only set when cookieConsent is true | 03-ARCHITECTURE.md §21 | PHP unit tests | `api/index.php` | gap |
| `02-§44.19` | CORS headers set for ALLOWED_ORIGIN and QA_ORIGIN | 03-ARCHITECTURE.md §21 | PHP unit tests | `api/index.php` | gap |
| `02-§44.20` | OPTIONS preflight returns 204 with CORS headers | 03-ARCHITECTURE.md §21 | PHP unit tests | `api/index.php` | gap |
| `02-§44.21` | Config from env vars (same names as Node.js) | 03-ARCHITECTURE.md §21 | manual: inspect code | `api/index.php` | gap |
| `02-§44.22` | Loads .env file at startup if it exists | 03-ARCHITECTURE.md §21 | manual: deploy and verify | `api/index.php` | gap |
| `02-§44.23` | Secrets never appear in error responses | 03-ARCHITECTURE.md §21 | PHP unit tests | `api/index.php` | gap |
| `02-§44.24` | PHP API lives in api/ at project root | 03-ARCHITECTURE.md §21 | manual: inspect structure | `api/` | gap |
| `02-§44.25` | Dependencies managed via Composer | 03-ARCHITECTURE.md §21 | manual: inspect | `api/composer.json` | gap |
| `02-§44.26` | Directory structure: index.php, src/, composer.json, .env | 03-ARCHITECTURE.md §21 | manual: inspect structure | `api/` | gap |
| `02-§44.27` | .htaccess routes all requests to index.php | 03-ARCHITECTURE.md §21 | manual: deploy and verify | `api/.htaccess` | gap |
...
| `02-§44.31` | .env on server managed manually, not in deploy archive | 04-OPERATIONS.md | manual: verify | — | gap |
...
| `02-§44.37` | 04-OPERATIONS.md documents PHP API | 04-OPERATIONS.md | manual: read doc | `docs/04-OPERATIONS.md` | gap |
| `02-§44.38` | 08-ENVIRONMENTS.md documents qa environment and secrets | 08-ENVIRONMENTS.md | manual: read doc | `docs/08-ENVIRONMENTS.md` | gap |
| `02-§44.39` | 03-ARCHITECTURE.md notes dual API architecture | 03-ARCHITECTURE.md §21 | manual: read doc | `docs/03-ARCHITECTURE.md` | gap |
...
39 requirements added for PHP API (02-§44.1–44.39):
  4 implemented (coexistence: 02-§44.33–44.36, existing behaviour unchanged).
  35 gaps (PHP code, deploy workflow, documentation updates).
```
