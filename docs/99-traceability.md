...
| `02-§19.1` | When validation passes and submission begins, all form inputs and the submit button are immediately disabled | 03-ARCHITECTURE.md §8 | — (manual: press Skicka and confirm all inputs are disabled before the modal opens) | — | gap |
| `02-§19.2` | Disabled form elements are visually distinct (reduced opacity / grayed out) | 03-ARCHITECTURE.md §8 | — (manual: confirm visual appearance of disabled fieldset) | — | gap |
| `02-§19.3` | The consent banner is shown while the form is locked, inserted after the disabled submit button | 03-ARCHITECTURE.md §8 | — (manual: confirm banner position when form is locked and consent not yet given) | — | gap |
| `02-§19.4` | After the user accepts or declines, the banner removes itself and submission continues | 03-ARCHITECTURE.md §8 | — (manual: confirm banner disappears before modal opens) | — | gap |
| `02-§19.5` | After consent is resolved, a modal dialog opens over the page before the fetch begins | 03-ARCHITECTURE.md §8 | — (manual: confirm modal opens immediately after consent banner resolves) | — | gap |
| `02-§19.6` | The modal displays a spinner and the text "Skickar till GitHub…" while the fetch is in progress | 03-ARCHITECTURE.md §8 | — (manual: confirm spinner and text are visible during submission) | — | gap |
| `02-§19.7` | The modal carries role="dialog", aria-modal="true", and aria-labelledby pointing to its heading | 03-ARCHITECTURE.md §8 | — (manual: inspect modal element in DevTools) | — | gap |
| `02-§19.8` | Keyboard focus is trapped inside the modal while it is open | 03-ARCHITECTURE.md §8 | — (manual: Tab through the modal — focus must not leave it) | — | gap |
| `02-§19.9` | The page behind the modal is not scrollable while the modal is open | 03-ARCHITECTURE.md §8 | — (manual: confirm body does not scroll when modal is open) | — | gap |
| `02-§19.10` | On success, the modal shows the title, confirmation text, "Gå till schemat →" link, and "Lägg till en till" button | 03-ARCHITECTURE.md §8 | — (manual: submit a valid form and confirm modal success content) | — | gap |
| `02-§19.11` | If the user declined cookie consent, the success modal shows a Swedish note about editing not being possible | 03-ARCHITECTURE.md §8 | — (manual: decline consent, submit, and confirm note appears in modal) | — | gap |
| `02-§19.12` | "Lägg till en till" closes the modal, resets the form, and re-enables all fields | 03-ARCHITECTURE.md §8 | — (manual: click "Lägg till en till" and confirm form is blank and enabled) | — | gap |
| `02-§19.13` | On error, the modal shows the error message and a "Försök igen" button | 03-ARCHITECTURE.md §8 | — (manual: simulate a server error and confirm modal error content) | — | gap |
| `02-§19.14` | "Försök igen" closes the modal and re-enables all form fields (input data preserved) | 03-ARCHITECTURE.md §8 | — (manual: click "Försök igen" and confirm form is enabled with data intact) | — | gap |
| `02-§19.15` | The modal uses only CSS custom properties from 07-DESIGN.md §7 — no hardcoded colors or spacing | 07-DESIGN.md §7 | — (code review: grep for hardcoded hex/px values in modal CSS) | — | gap |
| `02-§19.16` | The modal is implemented in vanilla JavaScript; no library or framework is added | 03-ARCHITECTURE.md §8 | — (code review: confirm no new npm dependencies for modal logic) | — | gap |
| `02-§19.17` | The existing #result section is removed; the modal is the sole post-submission feedback mechanism | 03-ARCHITECTURE.md §8 | — (manual: confirm no #result section in rendered lagg-till.html) | — | gap |
...
All 17 are browser-only UX behaviour; none can be unit-tested in Node.js.
All 17 are marked gap — implementation follows in Phase 4.
Previous note: 3 requirements added for cookie consent UX fixes (02-§18.37–39).
